...
function connectMtoN(a, b) {
  if (a instanceof Array) {
    ...
    try {
      ...
      throw new SyntaxError("Cannot connect: " + b);
    }
  } else {
    throw new SyntaxError("Cannot connect: " + a);
  }
}

const BarPipe = Object.create(null);

//todo untested
BarPipe[">"] = function (left, right) {
  connectMtoN(left, right);
  //todo, here I would like to return a complex object, so that the context can be saved and mutated
  return right;
};
...
//todo untested
BarPipe["bpm"] = function (ctx, bpm, heavy, barTree) {
  if (barTree.type !== "|")
...
async function interpretSyntacticNodes(node, table) {
  //bottom processed first
  let args = node.body;
  if (args)                          //if the node is a function or expression, process its arguments first
    args = interpretArray(args, ctx, table);
  const match = table[node.type];
  if (match)                                //try to execute the function using table and arguments
  //todo here I can unwrap complex string objects that will be used as argument strings.
    return await(args instanceof Array ? match(ctx, ...args) : match(ctx));
...
export class InfiniteSound extends AudioContext {
  ...
  static async load(sound) {
    ...
    if (!result) {
      result = parse(sound);
      const result2 = interpretSyntacticNodes(result, ListOps);
