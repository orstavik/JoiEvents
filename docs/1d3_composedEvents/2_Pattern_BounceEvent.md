...
## Event sequence of bounced events

It is important to notice that the event propagation sequence of `composed: true` and bounced `composed: false` events differ.

Event sequence `composed: true` events:
```
...
Event sequence of **bounced** `composed: false` events:
```
The DOM         || 1.   | 2.   | 3.   | 4.   | 5.   | 6.     
==========================================================
#main dom       ||      |      |      | cap2 |      | bub2
...
This provides a the potential for the following logic:

 * if `.preventDefault()` or a custom method such as `.stopBouncing()` is called on the first event object, then the task of bouncing the event is cancelled.
